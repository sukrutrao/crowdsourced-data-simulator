...
class Simulator:
	...
	def __init__(self, num_people, num_questions, options_per_question, answers_per_question="single", sparsity=0):
		...
		assert(answers_per_question == "single" or answers_per_question == "multiple")
	...
	def generate_ground_truths(self):
		ground_truths = np.zeros(self.num_q, options_per_question)
		for i in range(0,num_q):
			if self.answers_per_q == "single":
				answer_question_i = np.random.randint(0,options_per_question)
				ground_truths[i][answer_question_i] = 1
			else:
				for j in range(0,options_per_question):
	...
	def generate_data(self, types_of_people):
		...
		cumulative_proportions = self.get_cumulative_proportions(total_sum_of_proportions)
		data = np.zeros(self.num_p, self.num_q, self.options_per_q)
		for person in range(0,len(self.num_p)):
			type_of_person = self.get_type_of_person(cumulative_proportions, total_sum_of_proportions)
			prob_of_person = self.get_prob_of_person(types_of_people[type_of_person])
			if self.answers_per_q == "single":
				for question in range(0,len(self.num_q)):
					correct_answer = self.get_correct_answer(ground_truths, question)
					answer_of_person = self.get_answer_of_person(correct_answer, prob_of_person, self.options_per_q)
					data[person][question][answer_of_person] = 1
			else:
				for question in range(0,len(self.num_q)):
					correct_answer = self.get_correct_answer(ground_truths, question)
					for option in range(0,len(self.options_per_q)):
						answer_of_person = self.get_answer_of_person(correct_answer[option], prob_of_person, self.options_per_q)
	...
	def flatten_data(self, data, order_by, mode):
		...
		if len(np.shape(data) != 3):
			if self.answers_per_q == "multiple" or (self.answers_per_q == "single" and mode == "vector"):
				...
				for question in range(0, len(self.num_q)):
					result_list.append([question, np.where(data[question] == 1).tolist()])
		elif self.answers_per_q == "multiple" or (self.answers_per_q == "single" and mode == "vector"):
			if order_by == "person":
				for person in range(0, len(self.num_p)):
					for question in range(0, len(self.num_q)):
						result_list.append([person, question, data[person][question].tolist()])
			else:
				for question in range(0, len(self.num_q)):
					for person in range(0, len(self.num_p)):
						result_list.append([question, person, data[person][question].tolist()])
		else:
			if order_by == "person":
				for person in range(0, len(self.num_p)):
					for question in range(0, len(self.num_q)):
						result_list.append([person, question, np.where(data[person][question] == 1)[0].tolist()])
			else:
				for question in range(0, len(self.num_q)):
					for person in range(0, len(self.num_p)):
						result_list.append([question, person, np.where(data[person][question] == 1)[0].tolist()])
	...
	def flatten_list(self, input_list):
		...
		for i in range(0, len(input_list)):
			result_list.append(self.flatten_list_element[i])
		return result_list
	...
	def get_cumulative_proportions(self, total_sum_of_proportions):
		...
		cumulative_proportions = np.zeros(types_of_people)
		cumulative_proportions[0] = types_of_people[0]
		for i in range(1,len(types_of_people)):
			cumulative_proportions[i] = cumulative_proportions[i-1] + types_of_people[i]
	...
	def get_prob_of_person(self, person_type_obj):
		return np.random.uniform(person_type_obj.low, person_type_obj.high)
